...
{
//     $file = str_ireplace('.', '_' . $key . '.', config('ds_config.default_goods_image'));
    $file = config('ds_config.default_goods_image');
...
function get_member_avatar_for_id($id)
{
    if (file_exists(BASE_UPLOAD_PATH . '/' . ATTACH_AVATAR . '/avatar_' . $id . '.jpg')) {
        ...
        if (config('ds_config.default_user_portrait')) {
            return UPLOAD_SITE_URL . '/' . ATTACH_COMMON . '/' . config('ds_config.default_user_portrait');
        }
        else {
            return UPLOAD_SITE_URL . '/' . ATTACH_COMMON . '/' . 'default_user_portrait.gif';
        }
...
function get_store_logo($img, $type = 'store_avatar')
{
    if ($type == 'store_avatar') {
        if (empty($img)) {
            return UPLOAD_SITE_URL . '/' . ATTACH_COMMON . '/' . config('ds_config.default_store_avatar');
        }
        else {
            $linfo = explode('_', $img);
    ...
    elseif ($type == 'store_logo') {
        if (empty($img)) {
            return UPLOAD_SITE_URL . '/' . ATTACH_COMMON . '/' . config('ds_config.default_store_logo');
        }
        else {
            $linfo = explode('_', $img);
            return UPLOAD_SITE_URL . '/' . ATTACH_STORE . '/' . $linfo['0'] . '/' . $img;
...
function o2o_send_order($order_list){
    $o2o_socket_url=config('ds_config.o2o_socket_url');
    if(!$o2o_socket_url){
        return;
    }
