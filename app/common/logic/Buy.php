...
class  Buy {
            ...
            if ($store_model->isO2oSupport($this->_store_list[$store_id])) {
                ...
                if ($this->_store_list[$store_id]['store_o2o_distribute_type'] == 1) {
                    ...
                    if(intval(config('ds_config.dada_open'))==1){
                        ...
                        $result['store_o2o_fee_list'][$store_id] = floatval(config('ds_config.o2o_fee'));
                    }
                }
            } else {
    ...
    private function _createOrderStep4() {
        ...
        foreach ($store_cart_list as $store_id => $goods_list) {
            ...
            $required_goods_id=Db::name('goods')->where(array(array('store_id','=',$order['store_id']),array('goods_if_required','=',1)))->column('goods_id');
        ...
        if (!empty($notice_list)) {
            foreach ($notice_list as $code => $value) {
                ...
                \mall\queue\QueueClient::push('sendStoremsg', array(
                    'code' => $code, 'store_id' => key($value), 'param' => $temp['param'], 'weixin_param' => $temp['weixin_param'], 'ali_param' => $temp['ali_param']
                ));
            }
        }
    }
