                ...
                foreach ($weight_list as $store_id => $cargo_weight) {
                    ...
                    if ($store_model->isO2oSupport($store_info)) {
                        if ($store_info['store_o2o_distribute_type'] == 1) {
                            $data[$store_info['store_id']] = array('o2o_third'=>'','distributor_type' => 1, 'o2o_fee' => $store_info['store_o2o_fee']);
                        } else {
                            $data[$store_info['store_id']] = array('o2o_third'=>'','distributor_type' => 0, 'o2o_fee' => config('ds_config.o2o_fee'));
                        }
                        if (intval(config('ds_config.dada_open')) == 1 && $store_info['store_o2o_distribute_type'] == 0) {
                            ...
                            //如果是店铺配送
                            $dist2 = ($store_info['store_o2o_distribute_type'] == 1) ? $store_info['store_o2o_distance'] : config('ds_config.o2o_distance');
    ...
    public function rcbPay($order_list, $input, $buyer_info) {
        ...
        foreach ($order_list as $key => $order_info) {
            ...
            if ($available_rcb_amount >= $order_amount) {
                ...
                if (!$insert) {
                    throw new \think\Exception('记录订单充值卡支付日志出现错误', 10006);
                }
...
//                 if (empty($pd_payment_info)) return;
        ...
        foreach ($order_list as $order_info) {
            ...
            if ($available_pd_amount >= $order_amount) {
                ...
                $insert = $order_model->addOrderlog($data);
                if (!$insert) {
                    throw new \think\Exception('记录订单预存款支付日志出现错误', 10006);
                }
