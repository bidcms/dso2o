...
//        $condition = $this->_getRecursiveClass($condition);
    ...
    public function delGoods($condition) {
        ...
        if (!empty($goods_list)) {
            ...
            // 删除买家收藏表数据
            model('favorites')->delFavorites(array(array('fav_id','in', $goodsid_array), array('fav_type' ,'=', 'goods')));
            // 删除商品赠品
    ...
    public function getGoodsSpecListByCommonId($common_id) {
        ...
        if (empty($spec_list)) {
            $spec_array = $this->getGoodsList(array('goods_commonid' => $common_id), 'goods_spec,goods_price,goods_promotion_price,goods_id,store_id,goods_image,color_id');
