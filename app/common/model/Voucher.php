...
class Voucher extends BaseModel
{
       ...
       $list = Db::name('voucher')->alias('v')->join('store s','s.store_id=v.voucher_store_id')->join('vouchertemplate t','v.vouchertemplate_id=t.vouchertemplate_id')->where($where)->order('voucher_id desc')->paginate(['list_rows'=>$pagesize,'query' => request()->param()],false);
        ...
        if (!empty($list) && is_array($list)) {
            foreach ($list as $key => $val) {
                //代金券图片
                if (empty($val['vouchertemplate_customimg']) || !file_exists(BASE_UPLOAD_PATH . DIRECTORY_SEPARATOR . ATTACH_VOUCHER . DIRECTORY_SEPARATOR . $val['store_id'] . DIRECTORY_SEPARATOR . $val['vouchertemplate_customimg'])) {
                    $list[$key]['vouchertemplate_customimg'] = UPLOAD_SITE_URL . DIRECTORY_SEPARATOR . default_goodsimage(60);
                }
                else {
                    $list[$key]['vouchertemplate_customimg'] = UPLOAD_SITE_URL . DIRECTORY_SEPARATOR . ATTACH_VOUCHER . DIRECTORY_SEPARATOR . $val['store_id'] . DIRECTORY_SEPARATOR .$val['vouchertemplate_customimg'];
                }
                //代金券状态文字
        ...
        if (!empty($voucher_list) && is_array($voucher_list)) {
            foreach ($voucher_list as $k => $v) {
                if (!empty($v['vouchertemplate_customimg'])) {
                    $v['vouchertemplate_customimg'] = UPLOAD_SITE_URL . DIRECTORY_SEPARATOR . ATTACH_VOUCHER . DIRECTORY_SEPARATOR . $v['vouchertemplate_store_id'] . DIRECTORY_SEPARATOR . $v['vouchertemplate_customimg'];
                }
                else {
                    $v['vouchertemplate_customimg'] = UPLOAD_SITE_URL . DIRECTORY_SEPARATOR . default_goodsimage(240);
                }
    ...
    public function getVouchertemplateInfo($where = array(), $field = '*', $order = '')
    {
        ...
        if (!empty($info['vouchertemplate_customimg'])) {
            $info['vouchertemplate_customimg'] = UPLOAD_SITE_URL . DIRECTORY_SEPARATOR . ATTACH_VOUCHER . DIRECTORY_SEPARATOR . $info['vouchertemplate_store_id'] . DIRECTORY_SEPARATOR . $info['vouchertemplate_customimg'];
        }
        else {
            $info['vouchertemplate_customimg'] = UPLOAD_SITE_URL . DIRECTORY_SEPARATOR . default_goodsimage(240);
        }
