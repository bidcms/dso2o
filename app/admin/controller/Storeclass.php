        ...
        if (!request()->isPost()) {
            ...
            if ($result) {
                ...
                if (!empty($_FILES['pic']['name'])) {//上传图片
                    $upload_file = BASE_UPLOAD_PATH . DIRECTORY_SEPARATOR . ATTACH_STORECLASS;
                    if (!empty($_FILES['pic']['name'])) {//上传图片
                        ...
                        try {
                            validate(['image' => 'fileSize:' . ALLOW_IMG_SIZE . '|fileExt:' . ALLOW_IMG_EXT])
                                    ->check(['image' => $file]);
                            $file_name = \think\facade\Filesystem::putFileAs('', $file, 'storeclass-pic-' . $result . '.png');
                        } catch (\Exception $e) {
            ...
            if (!empty($_FILES['pic']['name'])) {//上传图片
                ...
                if (!empty($_FILES['pic']['name'])) {//上传图片
                    ...
                    try {
                        validate(['image' => 'fileSize:' . ALLOW_IMG_SIZE . '|fileExt:' . ALLOW_IMG_EXT])
                                ->check(['image' => $file]);
                        $file_name = \think\facade\Filesystem::putFileAs('', $file, 'storeclass-pic-' . $storeclass_id . '.png');
                    } catch (\Exception $e) {
                        $this->error($e->getMessage());
                    }
                }
