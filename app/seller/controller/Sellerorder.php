...
class Sellerorder extends BaseSeller {
    ...
    public function show_distributor() {
        ...
        View::assign('o2o_socket_url', config('ds_config.o2o_socket_url'));
    ...
    public function receipt_order() {
        ...
        try {
            ...
            //判断是否需要自动派单
            $order_deliver_data = $order_model->autoOrderDeliver($this->store_info, $order_info);
            if ($order_deliver_data) {
                if ($order_deliver_data['o2o_distributor_phone']) {
                    $mailtemplates_model = model('mailtemplates');
                    $tpl_info = $mailtemplates_model->getTplInfo(array('mailmt_code' => 'order_receipt'));
                    ...
                    //发送短信
                    $result = model('smslog')->sendSms($order_deliver_data['o2o_distributor_phone'], $smslog_param);
                }
    ...
    public function reject_order() {
        ...
        Db::startTrans();
        try {
            $result = model('order', 'logic')->changeOrderStateCancel($order_info, 'seller', $this->seller_info['seller_name'], '店铺拒绝订单',true);
            if (!$result['code']) {
